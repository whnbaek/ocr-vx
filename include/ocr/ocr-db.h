...
u8 ocrDbCreate_site(ocrGuid_t *db, void** addr, u64 len, u16 flags, ocrHint_t *hint,
		ocrInDbAllocator_t allocator, const char* file, int line);
#define ocrDbCreate(DB,ADDR,LEN,FLAGS,HINT,ALLOCATOR) ocrDbCreate_site(DB,ADDR,LEN,FLAGS,HINT,ALLOCATOR,__FILE__,__LINE__)
#else 
...
#define ocrDbDestroy(DB) ocrDbDestroy_site(DB, __FILE__, __LINE__)
#else
...
#define ocrDbRelease(DB) ocrDbRelease_site(DB, __FILE__, __LINE__)
#else
